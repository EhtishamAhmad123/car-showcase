...
const getImageUrl = (mainImage) => {
  ...
  if (typeof mainImage === 'string' && (mainImage.startsWith('http://') || mainImage.startsWith('https://'))) {
    return mainImage;
  }
  
  // If it starts with /cars/, return as is (this handles subfolders)
  if (typeof mainImage === 'string' && mainImage.startsWith('/cars/')) {
    return mainImage;
  }
  
  // If it's a relative path without /cars/, add it
  if (typeof mainImage === 'string' && !mainImage.startsWith('/')) {
    return '/cars/' + mainImage;
  }
